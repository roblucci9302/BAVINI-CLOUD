...
export class WorkbenchStore {
  ...
  constructor() {
    ...
    this.showWorkbench.subscribe((show) => {
      if (show && !this.#webcontainerInitialized) {
  ...
  /**
   * Initialize WebContainer and related stores.
   * This is called lazily when the workbench is first shown,
   * NOT on page load, to prevent the page freeze.
   */
  #initWebContainer(): void {
    if (this.#webcontainerInitialized) {
      return;
    }

    this.#webcontainerInitialized = true;

    // Initialize stores - this triggers the WebContainer boot
    this.#filesStore.init();
    this.#previewsStore.init();

    // Terminal store doesn't need explicit init - it boots on attachTerminal()
  }
