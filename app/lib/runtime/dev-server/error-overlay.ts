...
export function generateErrorOverlayCode(config: Partial<ErrorOverlayConfig> = {}): string {
  ...
  // Serialize the helper functions needed for DOM-safe rendering
  const escapeHTMLCode = escapeHTML.toString();
  const formatCodeFrameCode = formatCodeFrame.toString();
  const cleanStackCode = cleanStack.toString();
  const generateStylesCode = generateStyles.toString();

  return `
(function() {
  const OVERLAY_ID = 'bavini-error-overlay';

  // Helper functions for safe HTML generation
  const escapeHTML = ${escapeHTMLCode};
  const formatCodeFrame = ${formatCodeFrameCode};
  const cleanStack = ${cleanStackCode};
  const generateStyles = ${generateStylesCode};
  const DEFAULT_THEME = ${JSON.stringify(DEFAULT_THEME)};
  const config = ${JSON.stringify(fullConfig)};

  function generateOverlayHTML(errors, config) {
    const theme = { ...DEFAULT_THEME, ...config.theme };
    const error = errors[0];
    if (!error) return '';

    const fileInfo = error.id || (error.loc ? error.loc.file : null) || 'Unknown file';
    const location = error.loc ? error.loc.line + ':' + error.loc.column : '';

    const frameHTML = error.frame
      ? '<div class="frame"><div class="frame-header">Code Frame</div><div class="frame-code">' + formatCodeFrame(error.frame) + '</div></div>'
      : '';

    const stackHTML = error.stack
      ? '<div class="stack"><div class="stack-title">Stack Trace</div><div class="stack-trace">' + escapeHTML(cleanStack(error.stack)) + '</div></div>'
      : '';
    ...
    const dismissButton = config.dismissible
      ? '<button class="close-btn" onclick="this.getRootNode().host.remove()"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></button>'
      : '';

    return '<style>' + generateStyles(theme) + '</style>' +
      '<div class="overlay"><div class="container">' +
      '<div class="header"><h1 class="title"><svg class="icon" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>Build Error</h1>' + dismissButton + '</div>' +
      '<div class="content">' + pluginHTML +
      '<div class="file-info"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" opacity="0.5"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>' +
      '<span class="file-path">' + escapeHTML(fileInfo) + '</span>' +
      (location ? '<span class="location">:' + location + '</span>' : '') + '</div>' +
      '<pre class="message">' + escapeHTML(error.message) + '</pre>' +
      frameHTML + stackHTML +
      '<div class="tip"><svg class="tip-icon" viewBox="0 0 24 24"><path d="M9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1zm3-19C8.14 2 5 5.14 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.86-3.14-7-7-7zm2.85 11.1l-.85.6V16h-4v-2.3l-.85-.6C7.8 12.16 7 10.63 7 9c0-2.76 2.24-5 5-5s5 2.24 5 5c0 1.63-.8 3.16-2.15 4.1z"/></svg>' +
      '<span class="tip-text">Fix the error above and save the file. The page will automatically reload.</span></div>' +
      '</div></div></div>';
  }
  ...
  class ErrorOverlay extends HTMLElement {
    ...
    show(errors) {
      // DOM-safe approach: clear and rebuild using template
      const html = generateOverlayHTML(errors, config);
